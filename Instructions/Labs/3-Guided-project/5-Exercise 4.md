...
1. Visual Studio Code のエクスプローラーを使用してプロジェクトのディレクトリとファイルを確認し、ソース コードを理解します。 ![エクスプローラー内のファイル のスクリーンショット。](../../media/explorer-bot-file.png)
...
1. ngrok をインストールするには、これらの手順に従います。
   ...
   1. ngrok によって、インターネットからアプリにアクセスするために使用できる転送 URL が生成されます。
      ...
      ```
   1. `Ctrl + C` を選択して ngrok トンネルを切断します。
...
3. ブラウザーの [アプリのインストール] ダイアログで、**[追加]** を選択して Teams アプリをインストールします。  Teams によって、インストールされたボットとの会話が開かれます。 ![SupportCommandBotlocal の追加のスクリーンショット](../../media/add-supportcommandbotlocal.png)
4. コマンド `resetPassword` を入力するか選択します。
5. ボットがパスワード リセット手順を含むアダプティブ カードで応答することを確認します。 ![SupportCommandBotlocal の実行のスクリーンショット](../../media/supportcommandbotlocal-run.png)
